...
// 0G Galileo Testnet 网络配置
export const ZERO_G_TESTNET_CONFIG = {
  chainId: 16601,
  name: '0G-Galileo-Testnet',
  currency: 'OG',
  blockExplorer: 'https://chainscan-galileo.0g.ai',
  rpcUrl: 'https://evmrpc-testnet.0g.ai', // 请确认此RPC URL是否正确
};

// 钱包存储键名
export const WALLET_STORAGE_KEYS = {
  PRIVATE_KEY: '0g_wallet_private_key',
  ADDRESS: '0g_wallet_address',
  MNEMONIC: '0g_wallet_mnemonic',
} as const;
...
export async function getWalletBalance(address: string): Promise<string> {
  try {
    const provider = new ethers.JsonRpcProvider(ZERO_G_TESTNET_CONFIG.rpcUrl);
